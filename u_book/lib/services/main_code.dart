...
export default class extends Extension {
	async itemHome(url, page) {
		...
		for (const item of list) {
			...
			result.push({
				...
				description: await await this.querySelector(html, "div.chapter-item a").text,
				host: this.hostExt,
				cover,
			});
		}
	...
	async detail(url) {
		...
		const name = await this.querySelector(detailEl.content, "div.post-title h1")
			.text;
		var cover = await this.getAttributeText(detailEl.content, "div.summary_image img", "src");
		const authorRow = await this.querySelectorAll(
			detailEl.content,
			"div.post-content_item"
		);
		const author = await this.querySelector(authorRow[1].content, "div.summary-content").text;
		const description = await this.querySelector(
			detailEl.content,
	...
	async chapter(url) {
		const res = await this.request(url);
		const listEl = await this.querySelectorAll(res, "div.reading-content div.page-break");
		let result = [];
		for (const element of listEl) {
			var image = await this.getAttributeText(element.content, "img", "src");
			result.push({
